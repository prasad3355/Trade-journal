...
function PerformancePanel({ trades }) {
  ...
  const points = trades
    .slice()
    .reverse()
    .map((trade) => (runningPnl += trade.pnl));
  ...
  const coords = points
    .map(
      (value, index) =>
        `${20 + index * (340 / Math.max(points.length - 1, 1))},${142 - ((value - min) / range) * 104}`,
    )
    .join(" ");
  ...
  const breakdown = [
    ["Wins", wins, "profit"],
    ["Losses", losses, "loss"],
    ["Break-even", trades.length - wins - losses, ""],
    ["Long", long, ""],
    ["Short", short, ""],
  ];
    ...
    <section className="performance-layout">
      <article className="equity-card">
        <div className="panel-heading">
          ...
          <strong className={total >= 0 ? "profit" : "loss"}>
            {formatSigned(formatCurrency(total))}
          </strong>
        </div>
          ...
          >
            <defs>
              <linearGradient id="area" x1="0" x2="0" y1="0" y2="1">
                <stop stopColor="#57ca9a" stopOpacity=".34" />
                <stop offset="1" stopColor="#57ca9a" stopOpacity="0" />
              </linearGradient>
            </defs>
            <path
              d="M 0 145 H 380 M 0 90 H 380 M 0 35 H 380"
              className="equity-grid"
            />
            <polygon points={`20,150 ${coords} 360,150`} fill="url(#area)" />
      ...
      <article className="breakdown-card">
        ...
        <div className="breakdown-list">
          {breakdown.map(([label, value, tone]) => (
            <div key={label}>
              <span className={`breakdown-dot ${tone}`} /> <p>{label}</p>
              <strong>{value}</strong>
            </div>
          ))}
        </div>
